...
# Column used for OCR match overrides (e.g. bare "Kyūbi焔" → redirects to column B entry).
# Change this to any column letter to keep the front of the sheet clean.
OVERRIDE_COL = "C"
SOURCE_SHEET = "2026-1"
TARGET_SHEET = "2026-1"
...
def read_column_a_overrides(ws, roster: List[str]) -> Dict[str, str]:
    ...
    while row <= ws.max_row and roster_idx < len(roster):
        name = ws[f"B{row}"].value
        override_val = ws[f"{OVERRIDE_COL}{row}"].value  # <--- USE OVERRIDE_COL

        if name is not None and str(name).strip() in roster:
...
def try_direct_name_match(
    ...
    if key in matcher:
        ...
        if raw_name in aliases and aliases[raw_name] != matched:
            note = "Column A override (superseded alias)"
        return matched, note
...
# ============================================================================
# DATA AGGREGATION & MAIN EXECUTION FLOW
# ============================================================================
...
def main():
    ...
    overrides = read_column_a_overrides(target_ws, roster)
